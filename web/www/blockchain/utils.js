...
export class OrganizationClient extends EventEmitter {
  ...
  connectAndRegisterBlockEvent() {
    ...
    try {
      ...
      console.log(
        `Failed to connect and register block event. Error ${e.message}`
      );
      throw e;
    }
  }
  ...
  async checkChannelMembership() {
    try {
      ...
      return channels.some(
        ({ channel_id }) => channel_id === this._channelName
      );
    } catch (e) {
      return false;
    }
      ...
      const transactionCompletePromises = this._eventHubs.map(eh => {
        ...
        return new Promise((resolve, reject) => {
          ...
          eh.registerTxEvent(deployId, (tx, code) => {
            ...
            if (code != "VALID") {
              reject(
                new Error(`Peer has rejected transaction with code: ${code}`)
              );
            } else {
              resolve();
            }
