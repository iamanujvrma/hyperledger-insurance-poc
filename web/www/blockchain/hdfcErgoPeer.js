...
export async function fileClaim(claim) {
  ...
  try {
    ...
    console.log(
      `successResult, after calling invoke claim_file from insurancePeer.js ${util.inspect(
        successResult
      )}`
    );
    if (successResult) {
...
export async function authenticateUser(username, password) {
  ...
  try {
    let authenticated = await query("user_authenticate", {
      username,
      password
    });
