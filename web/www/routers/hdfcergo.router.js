...
router.post("/api/create-contract-type", async (req, res) => {
  ...
  if (
    !(typeof shopType === "string" && shopType[0]) ||
...
router.post("/api/set-contract-type-active", async (req, res) => {
  ...
  if (typeof uuid !== "string" || typeof active !== "boolean") {
    res.json({ error: "Invalid request." });
    return;
  }
  try {
    const success = await InsurancePeer.setActiveContractType(uuid, active);
    res.json({ success });
  } catch (e) {
    res.json({ error: "Error accessing blockchain." });
  }
